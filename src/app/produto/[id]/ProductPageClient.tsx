...
import { ChevronDown, Truck, Shield, Star, ArrowLeft, Check, Loader2 } from 'lucide-react';
...
export default function ProductPageClient({ product }: Props) {
  ...
  const [shippingResult, setShippingResult] = useState<{ cost: number; estimatedDays: number; label: string; free: boolean } | null>(null);
  ...
  const handleShipping = async () => {
    ...
    try {
      const res = await fetch(`/api/shipping?cep=${digits}&subtotal=${product.pixPrice}`);
  ...
  return (
    <>
      <ProductJsonLd product={product} url={`https://greekfw.com/produto/${product.id}`} />
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-12 py-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-14 xl:gap-24">
            ...
            <div className="lg:pt-4 space-y-8">
              ...
              {/* Logo variant */}
              <div>
                <p className="text-[#6F6A5F] text-[9px] tracking-[0.3em] uppercase mb-4 font-medium">Detalhe</p>
                <div className="flex gap-2">
                  {([
                    ...
                    >
                      <span className="block text-[9px] tracking-[0.3em] uppercase font-medium">{v.label}</span>
              ...
              <div className="border-t border-[#E6DFD2] pt-6">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    onChange={e => {
                      setCep(e.target.value.replace(/\D/g, '').replace(/^(\d{5})(\d)/, '$1-$2'));
                      setShippingResult(null);
                    }}
                    onKeyDown={e => e.key === 'Enter' && handleShipping()}
                  ...
                  <button
                    ...
                    className="text-[#A88F6A] text-[9px] tracking-[0.25em] uppercase border-b border-[#A88F6A]/30 hover:border-[#A88F6A] pb-2 transition-colors flex items-center gap-1"
                  >
                    {shippingLoading ? <Loader2 className="w-3 h-3 animate-spin" /> : 'Calcular'}
                  </button>
                </div>
                {shippingResult && (
                  <motion.div
                  ...
                  >
                    <span className={`font-light ${shippingResult.free ? 'text-[#A88F6A]' : 'text-[#1A1A1A]'}`}>
                      {shippingResult.label}
                    </span>
                    <span className="text-[#6F6A5F] font-light">
                      {shippingResult.estimatedDays} dias úteis
                    </span>
                  </motion.div>
                )}
              </div>
